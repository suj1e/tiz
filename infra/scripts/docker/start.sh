...
echo "=========================================="
echo "   Nexora Infrastructure Deployment"
...
$DOCKER_COMPOSE up -d mysql redis --pull never
...
# 检查MySQL
if $DOCKER_COMPOSE exec -T mysql mysqladmin ping -h localhost -u root -pNexora@2026 &> /dev/null; then
...
$DOCKER_COMPOSE up -d elasticsearch --pull never
...
# 检查Elasticsearch
for i in {1..10}; do
    if $DOCKER_COMPOSE exec -T elasticsearch curl -s -u elastic:Nexora@2026 http://localhost:9200/_cluster/health &> /dev/null; then
...
echo "=========================================="
echo "Starting Kibana, Nacos, Kafka..."
$DOCKER_COMPOSE up -d kibana nacos kafka --pull never
...
# 检查Nacos
if $DOCKER_COMPOSE exec -T nacos curl -s http://localhost:8080/ &> /dev/null; then
...
# 检查Kafka
if $DOCKER_COMPOSE exec -T kafka kafka-broker-api-versions --bootstrap-server localhost:9094 &> /dev/null; then
...
echo ""
echo "=========================================="
echo -e "${GREEN}Phase 4: Starting UI & Management Tools${NC}"
echo "=========================================="
echo "Starting Kafka UI, Tempo, Grafana..."
$DOCKER_COMPOSE up -d kafka-ui tempo grafana --pull never
...
echo "  Kibana:         http://localhost:30005"
echo "  Nacos:          http://localhost:30006 (nacos/nacos)"
echo "  Kafka:          localhost:30009"
echo "  Kafka UI:       http://localhost:30010"
echo "  Tempo:          http://localhost:30014"
echo "  Grafana:        http://localhost:30018 (admin/Nexora@2026)"
echo ""
echo -e "${YELLOW}Tip: Use './scripts/docker/status.sh' to check service health${NC}"
echo ""
