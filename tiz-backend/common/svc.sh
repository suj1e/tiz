#!/bin/bash
# Service Management Script for common
...
cmd_publish() {
    log_warn "Common does not have an API module to publish"
}
...
cmd_help() {
    ...
    echo ""
    echo "Note: Common has no API module, so 'publish' is not available."
}
