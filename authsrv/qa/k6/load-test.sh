...
usage() {
    ...
    echo ""
    echo "选项:"
    echo "  -u, --url URL            目标服务 URL (默认: http://localhost:40006)"
    echo "  -c, --context PATH       上下文路径 (默认: auth)"
    ...
    echo "  soak   - 浸泡测试 (100 VUs 持续 1hour) - 测试稳定性"
    ...
    echo ""
    echo "  # 负载测试"
    echo "  $0 -s load -u http://test-api.example.com"
    echo ""
    echo "  # 压力测试"
    echo "  $0 -s stress"
    echo ""
    ...
    echo "  CONTEXT_PATH    上下文路径"
    echo "  TEST_USERNAME   测试用户名 (默认: admin)"
    echo "  TEST_PASSWORD   测试密码 (默认: admin123)"
...
k6 run \
  ...
  -e TEST_USERNAME="${TEST_USERNAME:-admin}" \
  -e TEST_PASSWORD="${TEST_PASSWORD:-admin123}" \
  -e TOKEN="${TOKEN:-}" \
