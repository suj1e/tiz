...
/// Merges Chat and Commands into a single Claude CLI-style interface
/// Features:
...
class _AssistantTabState extends State<AssistantTab> {
  ...
  final List<AssistantMessage> _messages = [
    AssistantMessage.system(
      text: 'Tiz AI 助手已就绪。你可以提问或输入指令（以 / 开头）。',
  ...
  Widget _buildMessageItem(AssistantMessage message, ThemeColors colors) {
    switch (message.type) {
  ...
  void _sendChatMessage(String text) {
    setState(() {
      _messages.add(AssistantMessage.chat(
        text: text,
        isUser: true,
        timestamp: DateTime.now(),
      ));
  ...
  void _executeCommand(String command) {
    ...
    final cmdMessage = AssistantMessage.command(
      command: command,
    ...
    Future.doWhile(() async {
      ...
      setState(() {
        ...
        if (msgIndex >= 0) {
          _messages[msgIndex] = AssistantMessage.command(
            command: command,
      ...
      if (mounted) {
        setState(() {
          ...
          // Add system message
          _messages.add(AssistantMessage.system(
            text: '$command 已完成',
            timestamp: DateTime.now(),
          ));
        });
...
abstract class AssistantMessage {
  ...
  AssistantMessage({required this.type, required this.timestamp});

  factory AssistantMessage.chat({
    required String text,
    required bool isUser,
    required DateTime timestamp,
  }) => ChatMessage(text: text, isUser: isUser, timestamp: timestamp);

  factory AssistantMessage.command({
  ...
  factory AssistantMessage.system({
    required String text,
    required DateTime timestamp,
...
/// Chat message
class ChatMessage extends AssistantMessage {
  final String text;
  final bool isUser;
...
/// Command message
class CommandMessage extends AssistantMessage {
...
/// System message
class SystemMessage extends AssistantMessage {
  final String text;
